//! Selects which JS game and diagnostic mode get embedded into the EBOOT at
//! build time.
//!
//! Set `PSPJS_GAME` to a filename in `src/game/` (default `raw-snake.js`):
//!   PSPJS_GAME=raw-pong.js bun runtime/build.ts
//!
//! Set `PSPJS_DIAG_MODE=boot` for the real-device boot/GU smoke test, or
//! `PSPJS_DIAG_MODE=trace` for on-screen startup stage tracing.
//!
...
fn main() {
    ...
    let diag_mode = env::var("PSPJS_DIAG_MODE").unwrap_or_default();
    ...
    println!("cargo:rustc-env=PSPJS_DIAG_MODE={}", diag_mode);
    println!("cargo:rerun-if-env-changed=PSPJS_GAME");
    println!("cargo:rerun-if-env-changed=PSPJS_DIAG_MODE");
