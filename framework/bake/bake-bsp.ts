...
const outName = `assets-bsp-${ACTIVE.replace(/[^a-z0-9]/g, '-')}`;
const constName = 'BSP_' + ACTIVE.toUpperCase().replace(/[^A-Z0-9]/g, '_');
...
`;
  return { ts, meshesOut, totalV, totalT, walls: solidAABBs.length / 4 };
}
...
console.log(`  spawn=[${round(spawn[0])}, ${round(spawn[1])}] floorY=${round(floorY)} span=${round(span)} walls=${out.walls} minArea=${round(minArea)} m²${M.committed ? '' : '  (NOT committed — gitignore)'}`);
