...
const SPECS: { name: string; frames: number; input?: (f: number) => number }[] = [
  ...
  // tatical: skinned human-scale character walking through the richer outdoor scene.
  { name: "tatical3d", frames: 180, input: (f) => (f < 70 ? 0x10 : f < 120 ? 0x10 | 0x80 : 0x8000) | (f > 30 && f < 115 ? 0x4000 : 0) },
